...
const BookCreatePage: React.FC = () => {
    ...
    const [formData, setFormData] = useState<BookFormData>({ 
        ...
        author: '', 
        price: '', 
        categoryId: '', // ID danh mục sẽ được set sau khi load categories
        stock: '',
        imageUrl: '',
    });
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                // Giả định API getAllCategories trả về { data: Category[] }
                const res = await categoryService.getAllCategories(); 
                const loadedCategories: Category[] = res.data.data || res.data;
                setCategories(loadedCategories || []);
                
                // Thiết lập category mặc định đầu tiên nếu chưa có
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            navigate('/books'); // Chuyển về trang danh sách
        } catch (error) {
